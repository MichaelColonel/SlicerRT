...
void qSlicerRtImageModuleWidget::setParameterNode(vtkMRMLNode *node)
{
  ...
  // Children widgets must update their elements themselfs
...
void qSlicerRtImageModuleWidget::onEnter()
{
  ...
  if (!d->ParameterNode->GetBeamNode())
  {
    ...
    if (vtkMRMLNode* node = this->mrmlScene()->GetNthNodeByClass( 0, "vtkMRMLRTBeamNode"))
    {
      ...
      // Explicitly update normal and vup vectors for parameter node
      d->logic()->UpdateNormalAndVupVectors(d->ParameterNode);
    }
    else 
    {
      d->RtBeamNode = nullptr;
