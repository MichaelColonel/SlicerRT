...
OpcUaTreeItem::OpcUaTreeItem(QOpcUaNode *node, OpcUaModel *model, OpcUaTreeItem *parent) 
  :
    QObject(parent)
  , mOpcNode(node)
  , mModel(model)
...
OpcUaTreeItem::OpcUaTreeItem(QOpcUaNode *node, OpcUaModel *model, const QOpcUaReferenceDescription &browsingData, OpcUaTreeItem *parent)
  :
...
void OpcUaTreeItem::handleAttributes(QOpcUa::NodeAttributes attr)
{
  ...
  {
    mNodeDescription = mOpcNode->attribute(QOpcUa::NodeAttribute::Description).value<QOpcUaLocalizedText>().text();
  }
...
void OpcUaTreeItem::browseFinished(const QVector<QOpcUaReferenceDescription> &children, QOpcUa::UaStatusCode statusCode)
{
  ...
  for (const auto &item : children)
  {
    ...
    this->appendChild(new OpcUaTreeItem(node, mModel, item, this));
...
OpcUaTreeItem* OpcUaTreeItem::findParentItemByName(OpcUaTreeItem* parentItem, const QString& displayedName)
{
  if (!parentItem)
  {
    return nullptr;
  }
  if (parentItem->getNodeDisplayName() == displayedName)
  {
    return parentItem;
  }
  for (int i = 0; i < parentItem->childCount(); ++i)
  {
    OpcUaTreeItem* item = OpcUaTreeItem::findParentItemByName(parentItem->child(i), displayedName);
    if (item)
    {
      return item;
    }
...
//    }
  ...
  {
    for (int i = 0; i < item->currentChildCount(); ++i)
    {
      finalLeaves += OpcUaTreeItem::findLeafValueItems(item->child(i));
    }
  }
  return finalLeaves;
}

QStringList OpcUaTreeItem::findParentNamesForItem(OpcUaTreeItem* currentItem, OpcUaTreeItem* rootItem)
{
  ...
  if (currentItem == rootItem)
  {
    parentNames.push_back(rootItem->getNodeDisplayName());
    return parentNames;
  }
  if (OpcUaTreeItem* parent = currentItem->parentItem())
  {
    parentNames.push_back(currentItem->getNodeDisplayName());
    parentNames += OpcUaTreeItem::findParentNamesForItem(parent, rootItem);
  }
  return parentNames;
}
