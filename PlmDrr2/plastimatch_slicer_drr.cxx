...
plastimatch drr -A cpu \
	--nrm "0.980282 -0.052336 0.190547" \
	--vup "-0.0513744 -0.99863 -0.00998617" \
	--sad 1000 --sid 1300 \
	-r "2000 2000" \
	-z "500 500" \
	-c "922.5 969" \
	-o "1.43469 -20.7674 -49.7378" \
	-w "300 1546 180 1759" \
	-e -i exact -P preprocess -O Out -t raw

*/
/*

Input file: /tmp/Slicer-michel/inputVolumeFile.mha
Output prefix: Out
Output details prefix: 
Output details file name: 
plastimatch drr -A cpu \
have normal: 1
	--nrm "0.980282 -0.052336 0.190547" \
	--vup "-0.0513744 -0.99863 -0.00998617" \
...
*/
  ...
  options.image_resolution[0] = 2000;
  options.image_resolution[1] = 2000;
  options.detector_resolution[0] = 2000;
  options.detector_resolution[1] = 2000;
  options.image_size[0] = 500.;
  options.image_size[1] = 500.;
  options.image_window[0] = 0;
  options.image_window[1] = 1999;
  options.image_window[2] = 0;
  options.image_window[3] = 1999;
  options.image_center[0] = 999.; // columns
  options.image_center[1] = 999.; // rows
...
//  float imageCenterY = float(imageWindow[0]) + float(imageWindow[1] - imageWindow[0] - 1) / 2.f; // rows
  ...
  options.vup[0] = 0.0513744;
  options.vup[1] = -0.99863;
  options.vup[2] = -0.00998617;

  options.have_nrm = 1;
  options.nrm[0] = 0.980282;
  options.nrm[1] = -0.052336;
  options.nrm[2] = 0.190547;

  if (sourceImagerDistance < sourceAxisDistance)
  {
    throw std::string("SID is less than SAD");
  }
  options.sad = 1000.;//sourceAxisDistance;
  options.sid = 1300.;//sourceImagerDistance;

  // Isocenter DICOM (LPS) position
  options.isocenter[0] = 1.43469;//isocenterPosition[0];
  options.isocenter[1] = -20.7674;//isocenterPosition[1];
  options.isocenter[2] = -49.7378;//isocenterPosition[2];
  ...
  options.autoscale_range[0] = 0.;//autoscaleRange[0];
  options.autoscale_range[1] = 255.;//autoscaleRange[1];
...
int main( int argc, char * argv[] )
{
  ...
  try
  {
    ...
    case itk::ImageIOBase::UCHAR:
      res = DoIt( argc, argv, mhaFileName, static_cast<unsigned char>(0) );
      break;
    case itk::ImageIOBase::CHAR:
      res = DoIt( argc, argv, mhaFileName, static_cast<signed char>(0) );
      break;
    case itk::ImageIOBase::USHORT:
      res = DoIt( argc, argv, mhaFileName, static_cast<unsigned short>(0) );
      break;
    case itk::ImageIOBase::SHORT:
      res = DoIt( argc, argv, mhaFileName, static_cast<short>(0) );
      break;
    case itk::ImageIOBase::UINT:
      res = DoIt( argc, argv, mhaFileName, static_cast<unsigned int>(0) );
      break;
    case itk::ImageIOBase::INT:
      res = DoIt( argc, argv, mhaFileName, static_cast<int>(0) );
      break;
    case itk::ImageIOBase::ULONG:
      res = DoIt( argc, argv, mhaFileName, static_cast<unsigned long>(0) );
      break;
    case itk::ImageIOBase::LONG:
      res = DoIt( argc, argv, mhaFileName, static_cast<long>(0) );
      break;
    case itk::ImageIOBase::FLOAT:
      res = DoIt( argc, argv, mhaFileName, static_cast<float>(0) );
      break;
    case itk::ImageIOBase::DOUBLE:
      res = DoIt( argc, argv, mhaFileName, static_cast<double>(0) );
      break;
  ...
  if (res == EXIT_SUCCESS && !mhaFileName.empty())
  {
